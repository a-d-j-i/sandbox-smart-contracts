...
contract PolygonEstateTokenV1 is EstateBaseToken, Initializable, IPolygonEstateToken {
    ...
    function _addGamesToEstate(
        ...
        for (uint256 i; i < len; i++) {
            ...
            // TODO: maybe is better to add to free land and then take from free land with _addTilesFromFreeLand?
            TileWithCoordLib.TileWithCoord[] memory template = gameToken.getTemplate(gameData[i].gameId);
            require(map.isEqual(template), "game template don't match map");
        }
    }
}
